    ...
    paused(false), manualControl(true), joystick_topic("/teleop/twistCommand"), auto_topic("/mux/autoCommand"), base_frame("/bubbleRob"), reference_frame("/world")
{
    ...
    pointCloudSub = nh.subscribe("/depth_registered/points",1,&SimTasksEnv::pointCloudCallback,this);
    ...
    detectedFacesSub = nh.subscribe("/detected_faces",1,&SimTasksEnv::detectedFacesCallback,this);
    pathSub = nh.subscribe("/pathTurtlebot",1,&SimTasksEnv::detectedFacesCallback,this);
    batterySub = nh.subscribe("/mobile_base/sensors/core",1,&SimTasksEnv::batteryCallback,this);
    velPub = nh.advertise<geometry_msgs::Twist>(auto_topic,1);
    pathPub = nh.advertise<nav_msgs::Path>("/pathTurtlebot",1);
    explorationPub = nh.advertise<std_msgs::Bool>("/exploration",1);
}
...
void SimTasksEnv::publishExploration(const std_msgs::Bool & start_exploration) {
    explorationPub.publish(start_exploration);
}
...
void SimTasksEnv::detectedFacesCallback(const face_detect_base::ROI_Array msg) {
    if(msg.ROIs.size()!=0) {

        detected_faces=true;
        roi_faces=msg.ROIs[0];
    }
    else {
...
void SimTasksEnv::batteryCallback(const kobuki_msgs::SensorState msg) {
    battery_lvl=msg.battery;
}